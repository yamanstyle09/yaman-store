...
app.get('/api/orders/delivered', authenticateToken, (req, res) => {
  db.all("SELECT * FROM orders WHERE status = 'delivered' AND is_legacy = 0 AND (dhd_status_label NOT LIKE '%🧪%' OR dhd_status_label IS NULL) ORDER BY createdAt DESC", [], (err, rows) => {
    if (err) return res.status(500).json({ error: err.message });
    res.json(rows);
  });
});
...
app.get('/api/analytics/erp-summary', authenticateToken, requireAdmin, (req, res) => {
  ...
  const getDeliveredOrders = () => new Promise((resolve) => {
    db.get(`
      ...
      FROM orders 
      WHERE status = 'delivered' AND (dhd_status_label NOT LIKE '%🧪%' OR dhd_status_label IS NULL) ${dOrders}
    `, [], (err, row) => {
      if (row) {
      ...
      db.get(`
        ...
        FROM orders 
        WHERE (status = 'delivered' OR status = 'cancelled') AND is_legacy = 0 AND (dhd_status_label NOT LIKE '%🧪%' OR dhd_status_label IS NULL) ${dOrders}
      `, [], (errProfit, rowProfit) => {
